...
// --- Apify: scrape one LinkedIn profile (sync run, returns dataset items) ---
async function scrapeProfile(url) {
  const endpoint =
    `https://api.apify.com/v2/acts/${APIFY_ACTOR}/run-sync-get-dataset-items` +
    `?token=${encodeURIComponent(process.env.APIFY_TOKEN)}&timeout=240`;

  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), 250 * 1000);
  try {
    ...
    if (!resp.ok) {
      ...
      throw new Error(`Apify returned ${resp.status}: ${text.slice(0, 300)}`);
    }
