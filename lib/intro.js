...
const APIFY_MODE = 'Profile details no email ($4 per 1k)'; // cheapest mode; email not needed
...
async function scrapeProfiles(urls) {
  ...
  if (!Array.isArray(items) || items.length === 0) {
    throw new Error('Scrape finished but returned no profiles — check the URLs and try again');
  }
  return items;
}

// Match a scraped item back to a specific input URL by its /in/<slug>
function matchProfile(items, url) {
  const slug = publicIdOf(url);
  const idFields = (item) => {
  ...
  return items.find((item) => idFields(item).some((v) => v.includes(slug)));
}
...
async function handleGenerate(body) {
  try {
    ...
    // Scrape both profiles in a single run
    let items;
    try {
      items = await scrapeProfiles([url1, url2]);
    } catch (err) {
      console.error('Apify error:', err.message);
      return { status: 502, payload: { error: `Couldn't fetch the LinkedIn profiles. ${err.message}` } };
    }

    // Match each returned profile to its input URL; fall back to array order
    let rawA = matchProfile(items, url1);
    let rawB = matchProfile(items, url2);
    if (!rawA || !rawB || rawA === rawB) {
      rawA = rawA || items[0];
      rawB = rawB && rawB !== rawA ? rawB : items.find((i) => i !== rawA) || items[1];
    }
    if (!rawA || !rawB) {
      return { status: 502, payload: { error: 'Only one profile came back — check both URLs and try again.' } };
    }
